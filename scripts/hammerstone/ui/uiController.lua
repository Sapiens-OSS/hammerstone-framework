...
function uiController:removeSubview(parentView, viewToRemove)
	...
	local function removeAllChildren(viewToRemove)
		local viewInfos = allViews[viewToRemove]
...
function uiController:logHierarchy(view)
	...
	local function logView(view, index, indent)
		local viewInfos = allViews[view]
		...
		if viewInfos then
			...
			for i, subView in ipairs(viewInfos.subViews) do
				logView(subView, i, indent + 4, lastmodulePath)
			end
		end
	end
...
function uiController:searchViews(modulePathOrNil, mainModulePathOrNil, viewTypeOrNil, viewNameOrNil)
	local results = {}
...
function uiController:searchSubViews(parentView, modulePathOrNil, mainModulePathOrNil, viewTypeOrNil, viewNameOrNil)
	local results = {}

	local function search(parentView, modulePathOrNil, mainModulePathOrNil, viewTypeOrNil, viewNameOrNil)
		if parentView and allViews[parentView] then
	...
	search(parentView, modulePathOrNil, mainModulePathOrNil, viewTypeOrNil, viewNameOrNil)

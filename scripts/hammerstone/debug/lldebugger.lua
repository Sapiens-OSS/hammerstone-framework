...
local hmLog = mjrequire "hammerstone/logging"

local function getDebugConfig(modName)
    local modDir = getModDirectory(modName)
    local configFilename = modDir .. "/debug.config"
    
    if not fileUtils.fileExistsAtPath(configFilename) then return end

    local fileContent = fileUtils.getFileContents(configFilename)
    return json:decode(fileContent)
end

local ____debugConfig = nil
...
____modules = {
    ["init"] = function(...)
        local ____exports = {}
        function ____exports.init(modName, ...)
            ____debugConfig = getDebugConfig(modName)
        ...
        do
            ...
            local function getScriptRoots()
                if not scriptRoots then
                    scriptRoots = {}
                    local scriptRootsStr = ____debugConfig.scriptRoots
    ...
    ["send"] = function(...) 
        ...
        do
            ...
            local outputFilePath = ____debugConfig.outputFile
            local outputFile
    ...
    ["debugger"] = function(...) 
        ...
        do
            ...
            local inputFilePath = ____debugConfig.inputFile
            local inputFile
            ...
            local skipUnmappedLines = ____debugConfig.skipUnmappedLines
            ...
            local breakInCoroutines = ____debugConfig.breakInCoroutines
    ...
    ["lldebugger"] = function(...) 
        ...
        function ____exports.start()
            if not ____debugConfig then
                hmLog:error("Can't debug with no config file. Make sure you started the debugger!")
                error()
            end
            Debugger.debugGlobal(true)
        end
